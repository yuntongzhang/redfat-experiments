...
OPT_LEVEL="-O2"
GAMESS_OPT_LEVEL="-O1"
COMMON_FLAGS="-m64 -fPIE -fPIC -g -fno-inline -fno-omit-frame-pointer"
CC="$CC -std=gnu89 $COMMON_FLAGS"
CXX="$CXX $COMMON_FLAGS"
FC="$FC $COMMON_FLAGS"
LDOPT="-pie"

# monitor_wrapper = valgrind --leak-check=no --undef-value-errors=no \$command

cat << EOF > config/$NAME.cfg
monitor_wrapper = /home/yuntong/DrMemory-Linux-2.5.0/bin/drmemory -no_count_leaks -no_check_uninitialized -- \$command
ignore_errors = yes
tune          = base
ext           = $NAME
