...
CC_PATH=/usr/bin
BIN_LOWFAT_PATH=$PWD/../../../fyp-sem2/lowfat.bin.tmp/new
...
# # step (1) generate allowlist for train workload
# TRAIN_NAME=${NAME}-TRAIN-GEN
# cat << EOF > config/$TRAIN_NAME.cfg
# ignore_errors = yes
# tune          = base
# ext           = $TRAIN_NAME
...
# runspec -c $TRAIN_NAME -a build -I -l --size train -n $NUM_RUNS $@
...
#     if [ -x *.${TRAIN_NAME} ]
#     then
#         exe=$(find . -name *.${TRAIN_NAME} -type f)
#         exe_full=$BENCHMARKS_PATH/$exe_dir/$exe
#         # currently lowfat.bin only works in its directory
#         cd $BIN_LOWFAT_PATH
#         if ./lowfat.bin -Xallowlist-gen -Xreads $exe_full | grep -C 9 "num_patched_T3"
#         then
...
# # run benchmarks
# cd $SPEC2006_PATH
# rm config/$TRAIN_NAME.*
# cat << EOF > config/$TRAIN_NAME.cfg
# monitor_wrapper = LOWFAT_ALLOWLIST=$BENCHMARKS_PATH/\$benchmark/allowlist-train.txt \$command
# ignore_errors = yes
# tune          = base
# ext           = $TRAIN_NAME
# output_format = asc, Screen
...
# runspec --nobuild -c $TRAIN_NAME -a run -I -l --size train -n $NUM_RUNS $@


# step (2) generate allowlist for ref workload
REF_NAME=${NAME}-REF-GEN
cat << EOF > config/$REF_NAME.cfg
ignore_errors = yes
tune          = base
ext           = $REF_NAME
output_format = asc, Screen
...
# set up build directories
runspec -c $REF_NAME -a build -I -l --size ref -n $NUM_RUNS $@
# patch the executables
cd $BENCHMARKS_PATH
exe_dirs=$(find . -path "./*/exe" -type d)
...
rm config/$REF_NAME.*
cat << EOF > config/$REF_NAME.cfg
monitor_wrapper = LOWFAT_ALLOWLIST=$BENCHMARKS_PATH/\$benchmark/allowlist-ref.txt \$command
ignore_errors = yes
tune          = base
ext           = $REF_NAME
output_format = asc, Screen
...
runspec --nobuild -c $REF_NAME -a run -I -l --size ref -n $NUM_RUNS $@
