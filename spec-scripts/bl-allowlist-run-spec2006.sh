...
rm config/$GEN_NAME.*
cat << EOF > config/$GEN_NAME.cfg
monitor_wrapper = LOWFAT_ALLOWLIST=$BENCHMARKS_PATH/\$benchmark/allowlist.txt \$command
ignore_errors = yes
tune          = base
ext           = $GEN_NAME
output_format = asc, Screen
...
do
    ...
    then
        ...
        # currently lowfat.bin only works in its directory
        cd $BIN_LOWFAT_PATH
        temp=$BENCHMARKS_PATH/$exe_dir
        allowlist_path=${temp%/exe}/allowlist.txt
        if ./lowfat.bin -Xallowlist=$allowlist_path -Oelim=true -Obatch=30 -Omerge=true -Xreads -Xbase $exe_full | grep -C 9 "num_patched_T3"
        then
            echo -e "\n${GREEN}log${OFF}: Successfully patched the executable $exe_full\n"
            rm $exe_full
            mv a.out $exe_full
